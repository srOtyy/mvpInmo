...
export class InmueblesRxjsService extends BaseCrudService<IInmueble> {
  ...
  devolverCaracteristica(
    idImueble: number,
    caracteristica: string,
  ): string | number | boolean {
    const inmueble = this.$lista().find((i) => i.id === idImueble);
    if (inmueble) {
      const valor = inmueble.caracteristicas.find(
        (c) => c.clave == caracteristica,
      )?.valor;
      if (valor) {
        return valor;
      } else {
  ...
  obtenerArrayDeInmueblesInactivosPorId(arrayId: number[]): IInmueble[] {
    const inmuebles: IInmueble[] = [];
    for (let id of arrayId) {
      const inmuebleAux = this.obtenerInmueblePorId(id);
      if (inmuebleAux && !inmuebleAux.activo) {
        inmuebles.push(inmuebleAux);
      }
    }
    return inmuebles;
  }
