...
export class PropietarioRxjsService extends BaseCrudService<IPropietario> {
  ...
  ): Promise<IPropietario | undefined> {
    if (this.$lista().length === 0) {
  ...
  obtenerNombrePropietarioPorId(idPropietario: number): string {
    ...
    if (!propietario) {
      console.warn(
        `Propietario con ID ${idPropietario} no encontrado. Asegúrate de que la lista de propietarios esté cargada. ( agregar async/await a la función que llama a esta función)`,
      );
  ...
  obtenerArrayDeInmueblesPorId(id: number): number[] {
    ...
    if (propietario) {
      return propietario.listaInmuebles;
    } else {
      return [0, 0, 0, 0];
    }
  }
